export const translations = {
  en: {
    // Navigation
    nav: {
      home: 'Home',
      channels: 'Channels',
      iptv_reseller: 'IPTV reseller',
      features: 'Features',
      pricing: 'Pricing',
      faq: 'FAQ',
      start_free_trial: 'Start Free Trial'
    },
    // Hero Section
    hero: {
      title: 'Best IPTV Service in Europe',
      subtitle: '33,000+ Live Channels & 150,000+ Movies',
      description: 'Experience premium entertainment with NinhoTV. Fast European servers, HD quality, 24/7 support.',
      get_started: 'Get Started',
      start_trial: 'Start Free Trial',
      trusted_by: 'Trusted by 50,000+ European customers'
    },
    // Pricing
    pricing: {
      title: 'Simple Pricing',
      subtitle: 'Choose perfect plan for your entertainment needs',
      try_before_buy: 'Try Before You Buy',
      free_trial_desc: 'Get a FREE 24-HOUR TRIAL with full access',
      start_trial_now: 'Start Free Trial Now',
      choose_device_package: 'Choose Your Device Package',
      device_desc: 'Select how many devices you want to use simultaneously',
      selected: 'Selected',
      devices: 'devices',
      month: '/month',
      months: '/months',
      year: '/year',
      most_popular: 'MOST POPULAR',
      get_started: 'Get Started',
      secure_payment: 'Secure Payment',
      instant_activation: 'Instant Activation',
      cancel_anytime: 'Cancel Anytime',
      support_24_7: '24/7 Support',
      back_to_home: 'Back to Home'
    },
    // Features
    features: {
      title: 'Why Choose NinhoTV?',
      subtitle: 'The best IPTV experience with premium features',
      hd_quality: 'HD & 4K Quality',
      hd_desc: 'Crystal clear picture quality with HD and 4K streaming',
      uk_servers: 'Fast European Servers',
      uk_desc: 'Lightning-fast streaming from European-based servers',
      support: '24/7 Support',
      support_desc: 'Round-the-clock customer support',
      compatible: 'All Devices',
      compatible_desc: 'Works on all your favorite devices'
    },
    // FAQ
    faq: {
      title: 'Frequently Asked Questions',
      subtitle: 'Everything you need to know about NinhoTV',
      question1: 'What is IPTV?',
      answer1: 'IPTV (Internet Protocol Television) delivers TV content over the internet instead of traditional cable or satellite.',
      question2: 'How many devices can I use?',
      answer2: 'You can choose between 1, 2, or 3 device connections depending on your subscription plan.',
      question3: 'Is there a free trial?',
      answer3: 'Yes! We offer a 24-hour free trial with full access to all channels and features.',
      question4: 'What channels are included?',
      answer4: 'We offer 33,000+ live channels including European, US, sports, movies, and international channels.',
      still_have_questions: 'Still Have Questions?',
      contact_support: 'Our European support team is available 24/7 to help you',
      chat_whatsapp: 'Chat on WhatsApp',
      email_us: 'Email Us'
    },
    // Footer
    footer: {
      description: 'Premium IPTV service with 33,000+ live channels and 150,000+ movies. Fast European servers, 24/7 support.',
      pages: 'Pages',
      support: 'Support',
      legal: 'Legal',
      contact_us: 'contact us',
      all_rights_reserved: 'All rights reserved.',
      secure_payment: 'Secure Payment',
      uk_servers: 'European Servers',
      support_24_7: '24/7 Support'
    },
    // Common
    common: {
      language: 'Language',
      select_language: 'Select Language',
      close: 'Close',
      learn_more: 'Learn More',
      view_pricing: 'View Pricing',
      contact_us: 'Contact Us',
      subscribe_now: 'Subscribe Now'
    },
    // Free Trial
    free_trial: {
      title: 'Start Your <span class="text-red-500">FREE</span> Trial',
      subtitle: 'Experience best IPTV service in Europe with no commitment',
      trial_badge: '24-HOUR FREE TRIAL',
      try_before_buy: 'Try Before You Buy',
      trial_description: 'Get instant access to all our premium features completely free for 24 hours. No credit card required, no hidden fees, just pure entertainment.',
      start_trial_now: 'Start Your Free Trial Now',
      no_card_required: 'No credit card required • Instant activation • Cancel anytime',
      secure_title: '100% Secure',
      secure_desc: 'Your data is protected',
      instant_title: 'Instant Setup',
      instant_desc: 'Start watching immediately',
      support_title: '24/7 Support',
      support_desc: 'We\'re here to help',
      select_device: 'Select Your Device',
      firestick: 'Amazon Fire Stick',
      android_box: 'Android Box',
      smart_tv: 'Smart TV',
      android_phone: 'Android Phone',
      iphone_ipad: 'iPhone & iPad',
      pc_mac: 'PC & Mac',
      mag_box: 'MAG Box',
      other: 'Other',
      device_not_specified: 'Not specified',
      select_iptv_app: 'Select Your IPTV App',
      iptv_smarters: 'IPTV Smarters Pro',
      xciptv: 'XCIPTV',
      ibo_player: 'IBO Player',
      cap_player: 'CAP Player',
      hot_player: 'Hot Player',
      xtream_codes: 'Xtream Codes',
      tivimate: 'TiviMate',
      other_app: 'Other App',
      recommended: 'Recommended',
      specify_app_name: 'Please specify your app name',
      enter_app_name: 'Enter app name'
    }
  },
  fr: {
    // Navigation
    nav: {
      home: 'Accueil',
      channels: 'Chaînes',
      iptv_reseller: 'Revendeur IPTV',
      features: 'Fonctionnalités',
      pricing: 'Tarifs',
      faq: 'FAQ',
      start_free_trial: 'Essai Gratuit'
    },
    // Hero Section
    hero: {
      title: 'Meilleur Service IPTV en Europe',
      subtitle: '33,000+ Chaînes en Direct & 150,000+ Films',
      description: 'Découvrez un divertissement premium avec NinhoTV. Serveurs rapides en Europe, qualité HD, support 24/7.',
      get_started: 'Commencer',
      start_trial: 'Essai Gratuit',
      trusted_by: 'Approuvé par 50,000+ clients en Europe'
    },
    // Pricing
    pricing: {
      title: 'Tarifs Simples',
      subtitle: 'Choisissez le plan parfait pour vos besoins de divertissement',
      try_before_buy: 'Essayez Avant d\'Acheter',
      free_trial_desc: 'Obtenez un ESSAI GRATUIT de 24 HEURES avec accès complet',
      start_trial_now: 'Commencer l\'Essai Gratuit',
      choose_device_package: 'Choisissez Votre Pack Appareil',
      device_desc: 'Sélectionnez combien d\'appareils vous souhaitez utiliser simultanément',
      selected: 'Sélectionné',
      devices: 'appareils',
      month: '/mois',
      months: '/mois',
      year: '/an',
      most_popular: 'LE PLUS POPULAIRE',
      get_started: 'Commencer',
      secure_payment: 'Paiement Sécurisé',
      instant_activation: 'Activation Instantanée',
      cancel_anytime: 'Annulation à Tout Moment',
      support_24_7: 'Support 24/7',
      back_to_home: 'Retour à l\'Accueil'
    },
    // Features
    features: {
      title: 'Pourquoi Choisir NinhoTV?',
      subtitle: 'La meilleure expérience IPTV avec fonctionnalités premium',
      hd_quality: 'Qualité HD & 4K',
      hd_desc: 'Qualité d\'image cristalline avec streaming HD et 4K',
      uk_servers: 'Serveurs Rapides en Europe',
      uk_desc: 'Streaming ultra-rapide depuis des serveurs basés en Europe',
      support: 'Support 24/7',
      support_desc: 'Support client disponible 24h/24 et 7j/7',
      compatible: 'Tous les Appareils',
      compatible_desc: 'Fonctionne sur tous vos appareils préférés'
    },
    // FAQ
    faq: {
      title: 'Questions Fréquemment Posées',
      subtitle: 'Tout ce que vous devez savoir sur NinhoTV',
      question1: 'Qu\'est-ce que l\'IPTV?',
      answer1: 'L\'IPTV (Télévision sur Protocole Internet) diffuse du contenu TV sur internet au lieu du câble ou satellite traditionnel.',
      question2: 'Combien d\'appareils puis-je utiliser?',
      answer2: 'Vous pouvez choisir entre 1, 2 ou 3 connexions appareils selon votre plan d\'abonnement.',
      question3: 'Y a-t-il un essai gratuit?',
      answer3: 'Oui! Nous offrons un essai gratuit de 24 heures avec accès complet à toutes les chaînes et fonctionnalités.',
      question4: 'Quelles chaînes sont incluses?',
      answer4: 'Nous offrons 33,000+ chaînes en direct incluant Europe, US, sports, films et chaînes internationales.',
      still_have_questions: 'Avez-vous Encore des Questions?',
      contact_support: 'Notre support en Europe est disponible 24/7 pour vous aider',
      chat_whatsapp: 'Discuter sur WhatsApp',
      email_us: 'Nous Envoyer un Email'
    },
    // Footer
    footer: {
      description: 'Service IPTV premium avec 33,000+ chaînes en direct et 150,000+ films. Serveurs rapides en Europe, support 24/7.',
      pages: 'Pages',
      support: 'Support',
      legal: 'Légal',
      contact_us: 'Nous Contacter',
      all_rights_reserved: 'Tous droits réservés.',
      secure_payment: 'Paiement Sécurisé',
      uk_servers: 'Serveurs Europe',
      support_24_7: 'Support 24/7'
    },
    // Common
    common: {
      language: 'Langue',
      select_language: 'Sélectionner la Langue',
      close: 'Fermer',
      learn_more: 'En Savoir Plus',
      view_pricing: 'Voir les Tarifs',
      contact_us: 'Nous Contacter',
      subscribe_now: 'S\'abonner Maintenant'
    },
    // Free Trial
    free_trial: {
      title: 'Commencez Votre Essai <span class="text-red-500">GRATUIT</span>',
      subtitle: 'Expérimentez le meilleur service IPTV en Europe sans engagement',
      trial_badge: 'ESSAI GRATUIT DE 24 HEURES',
      try_before_buy: 'Essayez Avant d\'Acheter',
      trial_description: 'Obtenez un accès instantané à toutes nos fonctionnalités premium gratuitement pendant 24 heures. Aucune carte de crédit requise, aucun frais caché, juste du divertissement pur.',
      start_trial_now: 'Commencer Votre Essai Gratuit Maintenant',
      no_card_required: 'Aucune carte de crédit requise • Activation instantanée • Annulation à tout moment',
      secure_title: '100% Sécurisé',
      secure_desc: 'Vos données sont protégées',
      instant_title: 'Configuration Instantanée',
      instant_desc: 'Commencez à regarder immédiatement',
      support_title: 'Support 24/7',
      support_desc: 'Nous sommes là pour vous aider',
      select_device: 'Sélectionnez Votre Appareil',
      firestick: 'Amazon Fire Stick',
      android_box: 'Box Android',
      smart_tv: 'Smart TV',
      android_phone: 'Téléphone Android',
      iphone_ipad: 'iPhone & iPad',
      pc_mac: 'PC & Mac',
      mag_box: 'MAG Box',
      other: 'Autre',
      device_not_specified: 'Non spécifié',
      select_iptv_app: 'Sélectionnez Votre Application IPTV',
      iptv_smarters: 'IPTV Smarters Pro',
      xciptv: 'XCIPTV',
      ibo_player: 'IBO Player',
      cap_player: 'CAP Player',
      hot_player: 'Hot Player',
      xtream_codes: 'Xtream Codes',
      tivimate: 'TiviMate',
      other_app: 'Autre App',
      recommended: 'Recommandé',
      specify_app_name: 'Veuillez spécifier le nom de votre application',
      enter_app_name: 'Entrez le nom de l\'application'
    }
  },
  ar: {
    // Navigation
    nav: {
      home: 'الرئيسية',
      channels: 'القنوات',
      iptv_reseller: 'بائع IPTV',
      features: 'الميزات',
      pricing: 'الأسعار',
      faq: 'الأسئلة الشائعة',
      start_free_trial: 'تجربة مجانية'
    },
    // Hero Section
    hero: {
      title: 'أفضل خدمة IPTV في المملكة المتحدة',
      subtitle: '33,000+ قناة مباشرة و 150,000+ فيلم',
      description: 'استمتع بالترفيه المميز مع NinhoTV. خوادم سريعة في المملكة المتحدة، جودة HD، دعم على مدار الساعة.',
      get_started: 'ابدأ الآن',
      start_trial: 'تجربة مجانية',
      trusted_by: 'موثوق من قبل 50,000+ عميل في المملكة المتحدة'
    },
    // Pricing
    pricing: {
      title: 'أسعار بسيطة',
      subtitle: 'اختر الخطة المثالية لاحتياجات الترفيه الخاصة بك',
      try_before_buy: 'جرب قبل الشراء',
      free_trial_desc: 'احصل على تجربة مجانية لمدة 24 ساعة مع وصول كامل',
      start_trial_now: 'ابدأ التجربة المجانية',
      choose_device_package: 'اختر باقة الجهاز',
      device_desc: 'اختر عدد الأجهزة التي تريد استخدامها في نفس الوقت',
      selected: 'محدد',
      devices: 'أجهزة',
      month: '/شهر',
      months: '/أشهر',
      year: '/سنة',
      most_popular: 'الأكثر شعبية',
      get_started: 'ابدأ',
      secure_payment: 'دفع آمن',
      instant_activation: 'تنشيط فوري',
      cancel_anytime: 'إلغاء في أي وقت',
      support_24_7: 'دعم 24/7',
      back_to_home: 'العودة للرئيسية'
    },
    // Features
    features: {
      title: 'لماذا تختار NinhoTV؟',
      subtitle: 'أفضل تجربة IPTV مع ميزات مميزة',
      hd_quality: 'جودة HD و 4K',
      hd_desc: 'جودة صورة واضحة مع البث HD و 4K',
      uk_servers: 'خوادم سريعة في أوروبا',
      uk_desc: 'بث فائق السرعة من خوادم مقرها أوروبا',
      support: 'دعم 24/7',
      support_desc: 'دعم العملاء على مدار الساعة طوال أيام الأسبوع',
      compatible: 'جميع الأجهزة',
      compatible_desc: 'يعمل على جميع أجهزتك المفضلة'
    },
    // FAQ
    faq: {
      title: 'الأسئلة الشائعة',
      subtitle: 'كل ما تحتاج لمعرفته عن NinhoTV',
      question1: 'ما هو IPTV؟',
      answer1: 'IPTV (تلفزيون بروتوكول الإنترنت) يوفر محتوى تلفزيوني عبر الإنترنت بدلاً من الكابل أو القمر الصناعي التقليدي.',
      question2: 'كم عدد الأجهزة التي يمكنني استخدامها؟',
      answer2: 'يمكنك الاختيار بين 1 أو 2 أو 3 اتصالات أجهزة حسب خطة اشتراكك.',
      question3: 'هل هناك تجربة مجانية؟',
      answer3: 'نعم! نقدم تجربة مجانية لمدة 24 ساعة مع وصول كامل إلى جميع القنوات والميزات.',
      question4: 'ما هي القنوات المدرجة؟',
      answer4: 'نقدم 33,000+ قناة مباشرة بما في ذلك المملكة المتحدة والولايات المتحدة وأوروبا والرياضة والأفلام والقنوات الدولية.',
      still_have_questions: 'هل لديك أسئلة أخرى؟',
      contact_support: 'فريق الدعم في المملكة المتحدة متاح 24/7 لمساعدتك',
      chat_whatsapp: 'الدردشة عبر WhatsApp',
      email_us: 'أرسل لنا بريداً إلكترونياً'
    },
    // Footer
    footer: {
      description: 'خدمة IPTV مميزة مع 33,000+ قناة مباشرة و 150,000+ فيلم. خوادم سريعة في المملكة المتحدة، دعم 24/7.',
      pages: 'الصفحات',
      support: 'الدعم',
      legal: 'قانوني',
      contact_us: 'اتصل بنا',
      all_rights_reserved: 'جميع الحقوق محفوظة.',
      secure_payment: 'دفع آمن',
      uk_servers: 'خوادم أوروبا',
      support_24_7: 'دعم 24/7'
    },
    // Common
    common: {
      language: 'اللغة',
      select_language: 'اختر اللغة',
      close: 'إغلاق',
      learn_more: 'اعرف المزيد',
      view_pricing: 'عرض الأسعار',
      contact_us: 'اتصل بنا',
      subscribe_now: 'اشترك الآن'
    }
  },
  de: {
    // Navigation
    nav: {
      home: 'Startseite',
      channels: 'Kanäle',
      iptv_reseller: 'IPTV Wiederverkäufer',
      features: 'Funktionen',
      pricing: 'Preise',
      faq: 'FAQ',
      start_free_trial: 'Kostenlose Testversion'
    },
    // Hero Section
    hero: {
      title: 'Bester IPTV-Dienst in Europe',
      subtitle: '33,000+ Live-Kanäle & 150,000+ Filme',
      description: 'Erleben Sie Premium-Unterhaltung mit NinhoTV. Schnelle Europe-Server, HD-Qualität, 24/7-Support.',
      get_started: 'Loslegen',
      start_trial: 'Kostenlose Testversion',
      trusted_by: 'Vertraut von 50,000+ Europe-Kunden'
    },
    // Pricing
    pricing: {
      title: 'Einfache Preise',
      subtitle: 'Wählen Sie den perfekten Plan für Ihre Unterhaltungsbedürfnisse',
      try_before_buy: 'Testen Sie vor dem Kauf',
      free_trial_desc: 'Erhalten Sie eine KOSTENLOSE 24-STUNDEN-TESTVERSION mit vollem Zugriff',
      start_trial_now: 'Kostenlose Testversion starten',
      choose_device_package: 'Wählen Sie Ihr Gerätepaket',
      device_desc: 'Wählen Sie, wie viele Geräte Sie gleichzeitig verwenden möchten',
      selected: 'Ausgewählt',
      devices: 'Geräte',
      month: '/Monat',
      months: '/Monate',
      year: '/Jahr',
      most_popular: 'AM BELIEBTESTEN',
      get_started: 'Loslegen',
      secure_payment: 'Sichere Zahlung',
      instant_activation: 'Sofortige Aktivierung',
      cancel_anytime: 'Jederzeit kündbar',
      support_24_7: '24/7-Support',
      back_to_home: 'Zurück zur Startseite'
    },
    // Features
    features: {
      title: 'Warum NinhoTV wählen?',
      subtitle: 'Die beste IPTV-Erfahrung mit Premium-Funktionen',
      hd_quality: 'HD & 4K Qualität',
      hd_desc: 'Kristallklare Bildqualität mit HD- und 4K-Streaming',
      uk_servers: 'Schnelle Europe-Server',
      uk_desc: 'Blitzschnelles Streaming von Europe-basierten Servern',
      support: '24/7-Support',
      support_desc: 'Kundensupport rund um die Uhr',
      compatible: 'Alle Geräte',
      compatible_desc: 'Funktioniert auf all Ihren Lieblingsgeräten'
    },
    // FAQ
    faq: {
      title: 'Häufig gestellte Fragen',
      subtitle: 'Alles, was Sie über NinhoTV wissen müssen',
      question1: 'Was ist IPTV?',
      answer1: 'IPTV (Internet Protocol Television) liefert TV-Inhalte über das Internet anstelle von herkömmlichem Kabel oder Satellit.',
      question2: 'Wie viele Geräte kann ich verwenden?',
      answer2: 'Sie können zwischen 1, 2 oder 3 Geräteverbindungen je nach Ihrem Abonnementplan wählen.',
      question3: 'Gibt es eine kostenlose Testversion?',
      answer3: 'Ja! Wir bieten eine 24-stündige kostenlose Testversion mit vollem Zugriff auf alle Kanäle und Funktionen.',
      question4: 'Welche Kanäle sind enthalten?',
      answer4: 'Wir bieten 33,000+ Live-Kanäle einschließlich Europe, US, europäische, Sport, Filme und internationale Kanäle.',
      still_have_questions: 'Haben Sie noch Fragen?',
      contact_support: 'Unser Europe-Support-Team ist 24/7 verfügbar, um Ihnen zu helfen',
      chat_whatsapp: 'WhatsApp-Chat',
      email_us: 'E-Mail senden'
    },
    // Footer
    footer: {
      description: 'Premium IPTV-Dienst mit 33,000+ Live-Kanälen und 150,000+ Filmen. Schnelle Europe-Server, 24/7-Support.',
      pages: 'Seiten',
      support: 'Support',
      legal: 'Rechtliches',
      contact_us: 'Kontaktieren Sie uns',
      all_rights_reserved: 'Alle Rechte vorbehalten.',
      secure_payment: 'Sichere Zahlung',
      uk_servers: 'Europe-Server',
      support_24_7: '24/7-Support'
    },
    // Common
    common: {
      language: 'Sprache',
      select_language: 'Sprache auswählen',
      close: 'Schließen',
      learn_more: 'Mehr erfahren',
      view_pricing: 'Preise anzeigen',
      contact_us: 'Kontaktieren Sie uns',
      subscribe_now: 'Jetzt abonnieren'
    }
  },
  es: {
    // Navigation
    nav: {
      home: 'Inicio',
      channels: 'Canales',
      iptv_reseller: 'Revendedor IPTV',
      features: 'Características',
      pricing: 'Precios',
      faq: 'FAQ',
      start_free_trial: 'Prueba Gratuita'
    },
    // Hero Section
    hero: {
      title: 'Mejor Servicio IPTV en Europe',
      subtitle: '33,000+ Canales en Vivo & 150,000+ Películas',
      description: 'Experimenta entretenimiento premium con NinhoTV. Servidores rápidos en Europe, calidad HD, soporte 24/7.',
      get_started: 'Comenzar',
      start_trial: 'Prueba Gratuita',
      trusted_by: 'Confiado por 50,000+ clientes en Europe'
    },
    // Pricing
    pricing: {
      title: 'Precios Simples',
      subtitle: 'Elige el plan perfecto para tus necesidades de entretenimiento',
      try_before_buy: 'Prueba Antes de Comprar',
      free_trial_desc: 'Obtén una PRUEBA GRATUITA de 24 HORAS con acceso completo',
      start_trial_now: 'Iniciar Prueba Gratuita',
      choose_device_package: 'Elige Tu Paquete de Dispositivos',
      device_desc: 'Selecciona cuántos dispositivos quieres usar simultáneamente',
      selected: 'Seleccionado',
      devices: 'dispositivos',
      month: '/mes',
      months: '/meses',
      year: '/año',
      most_popular: 'MÁS POPULAR',
      get_started: 'Comenzar',
      secure_payment: 'Pago Seguro',
      instant_activation: 'Activación Instantánea',
      cancel_anytime: 'Cancelar en Cualquier Momento',
      support_24_7: 'Soporte 24/7',
      back_to_home: 'Volver al Inicio'
    },
    // Features
    features: {
      title: '¿Por qué elegir NinhoTV?',
      subtitle: 'La mejor experiencia IPTV con características premium',
      hd_quality: 'Calidad HD & 4K',
      hd_desc: 'Calidad de imagen cristalina con streaming HD y 4K',
      uk_servers: 'Servidores Rápidos en Europe',
      uk_desc: 'Streaming ultrarrápido desde servidores basados en Europe',
      support: 'Soporte 24/7',
      support_desc: 'Soporte al cliente disponible 24/7',
      compatible: 'Todos los Dispositivos',
      compatible_desc: 'Funciona en todos tus dispositivos favoritos'
    },
    // FAQ
    faq: {
      title: 'Preguntas Frecuentes',
      subtitle: 'Todo lo que necesitas saber sobre NinhoTV',
      question1: '¿Qué es IPTV?',
      answer1: 'IPTV (Televisión por Protocolo de Internet) entrega contenido de TV por internet en lugar de cable o satélite tradicional.',
      question2: '¿Cuántos dispositivos puedo usar?',
      answer2: 'Puedes elegir entre 1, 2 o 3 conexiones de dispositivos según tu plan de suscripción.',
      question3: '¿Hay una prueba gratuita?',
      answer3: '¡Sí! Ofrecemos una prueba gratuita de 24 horas con acceso completo a todos los canales y características.',
      question4: '¿Qué canales están incluidos?',
      answer4: 'Ofrecemos 33,000+ canales en vivo incluyendo Europe, US, europeos, deportes, películas y canales internacionales.',
      still_have_questions: '¿Aún tienes preguntas?',
      contact_support: 'Nuestro equipo de soporte en Europe está disponible 24/7 para ayudarte',
      chat_whatsapp: 'Chat por WhatsApp',
      email_us: 'Envíanos un Correo'
    },
    // Footer
    footer: {
      description: 'Servicio IPTV premium con 33,000+ canales en vivo y 150,000+ películas. Servidores rápidos en Europe, soporte 24/7.',
      pages: 'Páginas',
      support: 'Soporte',
      legal: 'Legal',
      contact_us: 'Contáctanos',
      all_rights_reserved: 'Todos los derechos reservados.',
      secure_payment: 'Pago Seguro',
      uk_servers: 'Servidores Europe',
      support_24_7: 'Soporte 24/7'
    },
    // Common
    common: {
      language: 'Idioma',
      select_language: 'Seleccionar Idioma',
      close: 'Cerrar',
      learn_more: 'Aprende Más',
      view_pricing: 'Ver Precios',
      contact_us: 'Contáctanos',
      subscribe_now: 'Suscribirse Ahora'
    },
    // Free Trial
    free_trial: {
      title: 'Inicia Tu Prueba <span class="text-red-500">GRATUITA</span>',
      subtitle: 'Experimenta el mejor servicio IPTV en Europa sin compromiso',
      trial_badge: 'PRUEBA GRATUITA DE 24 HORAS',
      try_before_buy: 'Prueba Antes de Comprar',
      trial_description: 'Obtén acceso instantáneo a todas nuestras características premium completamente gratis por 24 horas. No se requiere tarjeta de crédito, sin tarifas ocultas, solo entretenimiento puro.',
      start_trial_now: 'Iniciar Tu Prueba Gratuita Ahora',
      no_card_required: 'No se requiere tarjeta de crédito • Activación instantánea • Cancela en cualquier momento',
      secure_title: '100% Seguro',
      secure_desc: 'Tus datos están protegidos',
      instant_title: 'Configuración Instantánea',
      instant_desc: 'Comienza a ver inmediatamente',
      support_title: 'Soporte 24/7',
      support_desc: 'Estamos aquí para ayudarte',
      select_device: 'Selecciona Tu Dispositivo',
      firestick: 'Amazon Fire Stick',
      android_box: 'Box Android',
      smart_tv: 'Smart TV',
      android_phone: 'Teléfono Android',
      iphone_ipad: 'iPhone & iPad',
      pc_mac: 'PC & Mac',
      mag_box: 'MAG Box',
      other: 'Otro',
      device_not_specified: 'No especificado',
      select_iptv_app: 'Selecciona Tu Aplicación IPTV',
      iptv_smarters: 'IPTV Smarters Pro',
      xciptv: 'XCIPTV',
      ibo_player: 'IBO Player',
      cap_player: 'CAP Player',
      hot_player: 'Hot Player',
      xtream_codes: 'Xtream Codes',
      tivimate: 'TiviMate',
      other_app: 'Otra App',
      recommended: 'Recomendado',
      specify_app_name: 'Por favor especifica el nombre de tu app',
      enter_app_name: 'Ingresa el nombre de la app'
    }
  },
  nl: {
    // Navigation
    nav: {
      home: 'Startpagina',
      channels: 'Kanalen',
      iptv_reseller: 'IPTV Wederverkoper',
      features: 'Functies',
      pricing: 'Prijzen',
      faq: 'Veelgestelde Vragen',
      start_free_trial: 'Gratis Proefperiode'
    },
    // Hero Section
    hero: {
      title: 'Beste IPTV Dienst in Europa',
      subtitle: '33,000+ Live Kanalen & 150,000+ Films',
      description: 'Ervaar premium entertainment met NinhoTV. Snelle Europese servers, HD kwaliteit, 24/7 support.',
      get_started: 'Begin Nu',
      start_trial: 'Gratis Proefperiode',
      trusted_by: 'Vertrouwd door 50,000+ Europese klanten'
    },
    // Pricing
    pricing: {
      title: 'Eenvoudige Prijzen',
      subtitle: 'Kies het perfecte plan voor uw entertainment behoeften',
      try_before_buy: 'Probeer Voordat U Koopt',
      free_trial_desc: 'Krijg een GRATIS 24-UUR PROEFPERIODE met volledige toegang',
      start_trial_now: 'Start Gratis Proefperiode Nu',
      choose_device_package: 'Kies Uw Apparaat Pakket',
      device_desc: 'Selecteer hoeveel apparaten u tegelijk wilt gebruiken',
      selected: 'Geselecteerd',
      devices: 'apparaten',
      month: '/maand',
      months: '/maanden',
      year: '/jaar',
      most_popular: 'MEEST POPULAIR',
      get_started: 'Begin Nu',
      secure_payment: 'Veilige Betaling',
      instant_activation: 'Directe Activatie',
      cancel_anytime: 'Annuleer Elk Moment',
      support_24_7: '24/7 Support',
      back_to_home: 'Terug naar Startpagina'
    },
    // Features
    features: {
      title: 'Waarom NinhoTV Kiezen?',
      subtitle: 'De beste IPTV ervaring met premium functies',
      hd_quality: 'HD & 4K Kwaliteit',
      hd_desc: 'Kristalheldere beeldkwaliteit met HD en 4K streaming',
      uk_servers: 'Snelle Europese Servers',
      uk_desc: 'Bliksemsnelle streaming vanaf Europese servers',
      support: '24/7 Support',
      support_desc: 'Klantenservice de klok rond',
      compatible: 'Alle Apparaten',
      compatible_desc: 'Werkt op al uw favoriete apparaten'
    },
    // FAQ
    faq: {
      title: 'Veelgestelde Vragen',
      subtitle: 'Alles wat u moet weten over NinhoTV',
      question1: 'Wat is IPTV?',
      answer1: 'IPTV (Internet Protocol Televisie) levert TV content via internet in plaats van traditionele kabel of satelliet.',
      question2: 'Hoeveel apparaten kan ik gebruiken?',
      answer2: 'U kunt kiezen tussen 1, 2 of 3 apparaat verbindingen afhankelijk van uw abonnement plan.',
      question3: 'Is er een gratis proefperiode?',
      answer3: 'Ja! We bieden een gratis proefperiode van 24 uur met volledige toegang tot alle kanalen en functies.',
      question4: 'Welke kanalen zijn inbegrepen?',
      answer4: 'We bieden 33,000+ live kanalen inclusief Europese, Amerikaanse, sport, films en internationale kanalen.',
      still_have_questions: 'Heeft u Nog Vragen?',
      contact_support: 'Onze Europese support team is 24/7 beschikbaar om u te helpen',
      chat_whatsapp: 'Chat op WhatsApp',
      email_us: 'Stuur ons een Email'
    },
    // Footer
    footer: {
      description: 'Premium IPTV dienst met 33,000+ live kanalen en 150,000+ films. Snelle Europese servers, 24/7 support.',
      pages: 'Pagina\'s',
      support: 'Support',
      legal: 'Juridisch',
      contact_us: 'Neem Contact Op',
      all_rights_reserved: 'Alle rechten voorbehouden.',
      secure_payment: 'Veilige Betaling',
      uk_servers: 'Europese Servers',
      support_24_7: '24/7 Support'
    },
    // Common
    common: {
      language: 'Taal',
      select_language: 'Selecteer Taal',
      close: 'Sluiten',
      learn_more: 'Leer Meer',
      view_pricing: 'Bekijk Prijzen',
      contact_us: 'Neem Contact Op',
      subscribe_now: 'Abonneer Nu'
    },
    // Free Trial
    free_trial: {
      title: 'Start Uw <span class="text-red-500">GRATIS</span> Proefperiode',
      subtitle: 'Ervaar de beste IPTV dienst in Europa zonder verplichtingen',
      trial_badge: '24-UUR GRATIS PROEFPERIODE',
      try_before_buy: 'Probeer Voordat U Koopt',
      trial_description: 'Krijg direct toegang tot al onze premium functies helemaal gratis voor 24 uur. Geen credit card vereist, geen verborgen kosten, alleen pure entertainment.',
      start_trial_now: 'Start Uw Gratis Proefperiode Nu',
      no_card_required: 'Geen credit card vereist • Directe activatie • Annuleer elk moment',
      secure_title: '100% Veilig',
      secure_desc: 'Uw gegevens zijn beschermd',
      instant_title: 'Directe Installatie',
      instant_desc: 'Begin direct met kijken',
      support_title: '24/7 Support',
      support_desc: 'We zijn hier om u te helpen',
      select_device: 'Selecteer Uw Apparaat',
      firestick: 'Amazon Fire Stick',
      android_box: 'Android Box',
      smart_tv: 'Smart TV',
      android_phone: 'Android Telefoon',
      iphone_ipad: 'iPhone & iPad',
      pc_mac: 'PC & Mac',
      mag_box: 'MAG Box',
      other: 'Andere',
      device_not_specified: 'Niet gespecificeerd',
      select_iptv_app: 'Selecteer Uw IPTV App',
      iptv_smarters: 'IPTV Smarters Pro',
      xciptv: 'XCIPTV',
      ibo_player: 'IBO Player',
      cap_player: 'CAP Player',
      hot_player: 'Hot Player',
      xtream_codes: 'Xtream Codes',
      tivimate: 'TiviMate',
      other_app: 'Andere App',
      recommended: 'Aanbevolen',
      specify_app_name: 'Specificeer alsjebief de naam van uw app',
      enter_app_name: 'Voer de naam van de app in'
    }
  },
  sv: {
    // Navigation
    nav: {
      home: 'Hem',
      channels: 'Kanaler',
      iptv_reseller: 'IPTV-återförsäljare',
      features: 'Funktioner',
      pricing: 'Priser',
      faq: 'FAQ',
      start_free_trial: 'Starta Gratis Provperiod'
    },
    // Hero Section
    hero: {
      title: 'Bästa IPTV-tjänsten i Europa',
      subtitle: '33,000+ Live-kanaler & 150,000+ Filmer',
      description: 'Upplev premium underhållning med NinhoTV. Snabba europeiska servrar, HD-kvalitet, 24/7 support.',
      get_started: 'Kom Igång',
      start_trial: 'Gratis Provperiod',
      trusted_by: 'Betrodd av 50,000+ europeiska kunder'
    },
    // Pricing
    pricing: {
      title: 'Enkla Priser',
      subtitle: 'Välj det perfekta paketet för dina underhållningsbehov',
      try_before_buy: 'Prova Innan Du Köper',
      free_trial_desc: 'Få en GRATIS 24-TIMMARS PROVPERIOD med full tillgång',
      start_trial_now: 'Starta Gratis Provperiod Nu',
      choose_device_package: 'Välj Din Enhetspaket',
      device_desc: 'Välj hur många enheter du vill använda samtidigt',
      selected: 'Vald',
      devices: 'enheter',
      month: '/månad',
      months: '/månader',
      year: '/år',
      most_popular: 'MEST POPULÄR',
      get_started: 'Kom Igång',
      secure_payment: 'Säker Betalning',
      instant_activation: 'Omedelbar Aktivering',
      cancel_anytime: 'Avsluta När Som Helst',
      support_24_7: '24/7 Support',
      back_to_home: 'Tillbaka till Hem'
    },
    // Features
    features: {
      title: 'Varför Välja NinhoTV?',
      subtitle: 'Bästa IPTV-upplevelsen med premiumfunktioner',
      hd_quality: 'HD & 4K Kvalitet',
      hd_desc: 'Kristallklar bildkvalitet med HD och 4K-streaming',
      uk_servers: 'Snabba Europeiska Servrar',
      uk_desc: 'Blixtsnabb streaming från europeiska servrar',
      support: '24/7 Support',
      support_desc: 'Kundsupport dygnet runt',
      compatible: 'Alla Enheter',
      compatible_desc: 'Fungerar på alla dina favoritenheter'
    },
    // FAQ
    faq: {
      title: 'Vanliga Frågor',
      subtitle: 'Allt du behöver veta om NinhoTV',
      question1: 'Vad är IPTV?',
      answer1: 'IPTV (Internet Protocol Television) levererar TV-innehåll via internet istället för traditionell kabel eller satellit.',
      question2: 'Hur många enheter kan jag använda?',
      answer2: 'Du kan välja mellan 1, 2 eller 3 enhetsanslutningar beroende på din prenumerationsplan.',
      question3: 'Finns det en gratis provperiod?',
      answer3: 'Ja! Vi erbjuder en 24-timmars gratis provperiod med full tillgång till alla kanaler och funktioner.',
      question4: 'Vilka kanaler ingår?',
      answer4: 'Vi erbjuder 33,000+ live-kanaler inklusive europeiska, amerikanska, sport, film och internationella kanaler.',
      still_have_questions: 'Har Du Fler Frågor?',
      contact_support: 'Vår europeiska supportteam är tillgänglig 24/7 för att hjälpa dig',
      chat_whatsapp: 'Chatta på WhatsApp',
      email_us: 'Skicka E-post till Oss'
    },
    // Footer
    footer: {
      description: 'Premium IPTV-tjänst med 33,000+ live-kanaler och 150,000+ filmer. Snabba europeiska servrar, 24/7 support.',
      pages: 'Sidor',
      support: 'Support',
      legal: 'Juridiskt',
      contact_us: 'Kontakta Oss',
      all_rights_reserved: 'Alla rättigheter reserverade.',
      secure_payment: 'Säker Betalning',
      uk_servers: 'Europeiska Servrar',
      support_24_7: '24/7 Support'
    },
    // Common
    common: {
      language: 'Språk',
      select_language: 'Välj Språk',
      close: 'Stäng',
      learn_more: 'Läs Mer',
      view_pricing: 'Visa Priser',
      contact_us: 'Kontakta Oss',
      subscribe_now: 'Prenumerera Nu'
    },
    // Free Trial
    free_trial: {
      title: 'Starta Din <span class="text-red-500">GRATIS</span> Provperiod',
      subtitle: 'Upplev den bästa IPTV-tjänsten i Europa utan förpliktelser',
      trial_badge: '24-TIMMARS GRATIS PROVPERIOD',
      try_before_buy: 'Prova Innan Du Köper',
      trial_description: 'Få omedelbar tillgång till alla våra premiumfunktioner helt gratis i 24 timmar. Inget kreditkort krävs, inga dolda avgifter, bara ren underhållning.',
      start_trial_now: 'Starta Din Gratis Provperiod Nu',
      no_card_required: 'Inget kreditkort krävs • Omedelbar aktivering • Avsluta när som helst',
      secure_title: '100% Säker',
      secure_desc: 'Dina data är skyddade',
      instant_title: 'Omedelbar Installation',
      instant_desc: 'Börja titta direkt',
      support_title: '24/7 Support',
      support_desc: 'Vi är här för att hjälpa dig',
      select_device: 'Välj Din Enhet',
      firestick: 'Amazon Fire Stick',
      android_box: 'Android Box',
      smart_tv: 'Smart TV',
      android_phone: 'Android-telefon',
      iphone_ipad: 'iPhone & iPad',
      pc_mac: 'PC & Mac',
      mag_box: 'MAG Box',
      other: 'Annan',
      device_not_specified: 'Ej specificerad',
      select_iptv_app: 'Välj Din IPTV-app',
      iptv_smarters: 'IPTV Smarters Pro',
      xciptv: 'XCIPTV',
      ibo_player: 'IBO Player',
      cap_player: 'CAP Player',
      hot_player: 'Hot Player',
      xtream_codes: 'Xtream Codes',
      tivimate: 'TiviMate',
      other_app: 'Annan App',
      recommended: 'Rekommenderad',
      specify_app_name: 'Ange namnet på din app',
      enter_app_name: 'Ange appnamn'
    }
  },
  no: {
    nav: {
      home: 'Hjem',
      channels: 'Kanaler',
      iptv_reseller: 'IPTV-forhandler',
      features: 'Funksjoner',
      pricing: 'Priser',
      faq: 'FAQ',
      start_free_trial: 'Start Gratis Prøveperiode',
      contact: 'Kontakt'
    },
    hero: {
      title: 'Beste IPTV-tjeneste i Europa',
      subtitle: '33 000+ Live-kanaler & 150 000+ Filmer',
      description: 'Opplev premium underholdning med NinhoTV. Raske europeiske servere, HD-kvalitet, 24/7 support.',
      get_started: 'Kom i Gang',
      start_trial: 'Gratis Prøveperiode',
      trusted_by: 'Stolt av 50 000+ europeiske kunder'
    },
    pricing: {
      title: 'Enkle Priser',
      subtitle: 'Velg den perfekte planen for dine underholdningsbehov',
      try_before_buy: 'Prøv Før Du Kjøper',
      free_trial_desc: 'Få en GRATIS 24-TIMERS PRØVEPERIODE med full tilgang',
      start_trial_now: 'Start Gratis Prøveperiode Nå',
      choose_device_package: 'Velg Din Enhetspakke',
      device_desc: 'Velg hvor mange enheter du vil bruke samtidig',
      selected: 'Valgt',
      devices: 'enheter',
      month: '/måned',
      months: '/måneder',
      year: '/år',
      most_popular: 'MEST POPULÆR',
      get_started: 'Kom i Gang',
      secure_payment: 'Sikker Betaling',
      instant_activation: 'Øyeblikkelig Aktivering',
      cancel_anytime: 'Avslutt Når Som Helst',
      support_24_7: '24/7 Support',
      back_to_home: 'Tilbake til Hjem'
    },
    features: {
      title: 'Hvorfor Velge NinhoTV?',
      subtitle: 'Beste IPTV-opplevelse med premium-funksjoner',
      hd_quality: 'HD & 4K Kvalitet',
      hd_desc: 'Krystallklar bildekvalitet med HD og 4K-streaming',
      uk_servers: 'Raske Europeiske Servere',
      uk_desc: 'Lynrask streaming fra europeiske servere',
      support: '24/7 Support',
      support_desc: 'Kundestøtte døgnet rundt',
      compatible: 'Alle Enheter',
      compatible_desc: 'Fungerer på alle dine favorittenheter'
    },
    faq: {
      title: 'Ofte Stilte Spørsmål',
      subtitle: 'Alt du trenger å vite om NinhoTV',
      question1: 'Hva er IPTV?',
      answer1: 'IPTV (Internet Protocol Television) leverer TV-innhold over internett i stedet for tradisjonell kabel eller satellitt.',
      question2: 'Hvor mange enheter kan jeg bruke?',
      answer2: 'Du kan velge mellom 1, 2 eller 3 enhetstilkoblinger avhengig av abonnementsplanen din.',
      question3: 'Er det en gratis prøveperiode?',
      answer3: 'Ja! Vi tilbyr en 24-timers gratis prøveperiode med full tilgang til alle kanaler og funksjoner.',
      question4: 'Hvilke kanaler er inkludert?',
      answer4: 'Vi tilbyr 33 000+ live-kanaler inkludert europeiske, amerikanske, sport, filmer og internasjonale kanaler.',
      still_have_questions: 'Har Du Flere Spørsmål?',
      contact_support: 'Vårt europeiske supportteam er tilgjengelig 24/7 for å hjelpe deg',
      chat_whatsapp: 'Chat på WhatsApp',
      email_us: 'Send oss en E-post'
    },
    footer: {
      description: 'Premium IPTV-tjeneste med 33 000+ live-kanaler og 150 000+ filmer. Raske europeiske servere, 24/7 support.',
      pages: 'Sider',
      support: 'Support',
      legal: 'Juridisk',
      contact_us: 'Kontakt Oss',
      all_rights_reserved: 'Alle rettigheter forbeholdt.',
      secure_payment: 'Sikker Betaling',
      uk_servers: 'Europeiske Servere',
      support_24_7: '24/7 Support'
    },
    common: {
      language: 'Språk',
      select_language: 'Velg Språk',
      close: 'Lukk',
      learn_more: 'Les Mer',
      view_pricing: 'Se Priser',
      contact_us: 'Kontakt Oss',
      subscribe_now: 'Abonner Nå'
    },
    free_trial: {
      title: 'Start Din <span class="text-red-500">GRATIS</span> Prøveperiode',
      subtitle: 'Opplev den beste IPTV-tjenesten i Europa uten forpliktelser',
      trial_badge: '24-TIMERS GRATIS PRØVEPERIODE',
      try_before_buy: 'Prøv Før Du Kjøper',
      trial_description: 'Få umiddelbar tilgang til alle våre premium-funksjoner helt gratis i 24 timer. Ingen kredittkort kreves, ingen skjulte avgifter, bare ren underholdning.',
      start_trial_now: 'Start Din Gratis Prøveperiode Nå',
      no_card_required: 'Ingen kredittkort kreves • Øyeblikkelig aktivering • Avslutt når som helst',
      secure_title: '100% Sikker',
      secure_desc: 'Dataene dine er beskyttet',
      instant_title: 'Øyeblikkelig Oppsett',
      instant_desc: 'Begynn å se umiddelbart',
      support_title: '24/7 Support',
      support_desc: 'Vi er her for å hjelpe deg',
      select_device: 'Velg Din Enhet',
      firestick: 'Amazon Fire Stick',
      android_box: 'Android Box',
      smart_tv: 'Smart TV',
      android_phone: 'Android-telefon',
      iphone_ipad: 'iPhone & iPad',
      pc_mac: 'PC & Mac',
      mag_box: 'MAG Box',
      other: 'Annen',
      device_not_specified: 'Ikke spesifisert',
      select_iptv_app: 'Velg Din IPTV-app',
      iptv_smarters: 'IPTV Smarters Pro',
      xciptv: 'XCIPTV',
      ibo_player: 'IBO Player',
      cap_player: 'CAP Player',
      hot_player: 'Hot Player',
      xtream_codes: 'Xtream Codes',
      tivimate: 'TiviMate',
      other_app: 'Annen App',
      recommended: 'Anbefalt',
      specify_app_name: 'Vennligst oppgi navnet på appen din',
      enter_app_name: 'Skriv inn appnavn'
    }
  }
};

export type Language = keyof typeof translations;
export type TranslationKey = keyof typeof translations.en;
