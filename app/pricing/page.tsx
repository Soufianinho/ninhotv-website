...
export default function PricingPage() {
  ...
  const plans = pricingOptions.map((plan) => ({
    ...
    originalPrice: plan.originalPrice ? formatPrice(plan.originalPrice * devicePricing[selectedDevices as keyof typeof devicePricing], language) : undefined,
    features: [
  ...
  const handleSubscribe = (planName: string) => {
    ...
    window.open(`https://wa.me/212644870099?text=Hi! I want to subscribe to the ${planName} plan for ${selectedDevices} device${selectedDevices > 1 ? 's' : ''} at ${price}.`, '_blank');
  };
